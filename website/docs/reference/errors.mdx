...
## `TaggedError.adapt(...adapters)`

Builds a `catch` handler for `Result.try` and `Result.tryPromise` from `[from, to]` pairs:

```ts
TaggedError.adapt([ExternalNotFound, NotFound], [ExternalTimeout, Timeout]);
// (cause: unknown) => NotFound | Timeout | UnhandledException
```

| Rule         | Behavior                                                                                                            |
| ------------ | ------------------------------------------------------------------------------------------------------------------- |
| Match        | `cause instanceof from`; the first matching pair wins                                                               |
| Construction | `new to({ cause, message: cause.message })`                                                                         |
| No match     | `new UnhandledException({ cause })`, including non-Error thrown values                                              |
| `from`       | any `Error` class                                                                                                   |
| `to`         | a `TaggedError` class whose props need no more than `cause` and `message`; `cause` may be typed as the `from` class |
...
Returned by `Result.try` and `Result.tryPromise` when no custom catch translator is supplied, and by `TaggedError.adapt` when no pair matches.
